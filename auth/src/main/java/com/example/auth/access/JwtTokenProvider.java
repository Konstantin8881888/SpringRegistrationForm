...
public class JwtTokenProvider
{
    ...
    public String createAuthToken(String userName, String role)
    {
        ...
        Date now = new Date();
        Date valet = new Date(now.getTime() + getAuthExpirationCookie());
        return Jwts.builder().setClaims(claims).setIssuedAt(now).setExpiration(valet).signWith(SignatureAlgorithm.ES256, secretKey).compact();
    }
    ...
    public String createRefreshToken(String userName, String role)
    {
        ...
        claims.put("role", role);
        Date now = new Date();
        Date valet = new Date(now.getTime() + getRefreshExpirationCookie());
        return Jwts.builder().setClaims(claims).setIssuedAt(now).setExpiration(valet).signWith(SignatureAlgorithm.ES256, secretKey).compact();
    }
